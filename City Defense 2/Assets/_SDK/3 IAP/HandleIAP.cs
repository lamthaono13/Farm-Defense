...
public class HandleIAP : unity_base.Singleton<HandleIAP>
{
    ...
    public void BuyProduct(ShopProductNames productName)
    {
        ...
        HandleFireBase.Instance.LogEventWithParameter("Purchase_Click", new FirebaseParam[] { new FirebaseParam("name", productName) });
    }
    void OnBuyCompleted(IAPOperationStatus status, string message, StoreProduct product)
    {
        ...
        HandleFireBase.Instance.LogEventWithParameter("Purchase_Complete", new FirebaseParam[] { new FirebaseParam("name", product.productName) });
    }
