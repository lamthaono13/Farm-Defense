...
namespace Yade.Editor
{
    public partial class YadeSheet
    {
        ...
        private void OnHorizontalScroll(float value)
        {
            ...
            if (normalizedOffset > 0.99999999)
            {
        ...
        private void Paste()
        {
            ...
            if (!string.IsNullOrWhiteSpace(clipboardData))
            {
                ...
                // So we used \n to split lines
                var lines = clipboardData.Split(new char[] { '\n' });
                if (lines.Length > 0)
                {
                ...
                for (int index = 0; index < lines.Length; index++)
                {
                    ...
                    for (int columnIndex = 0; columnIndex < columns.Length; columnIndex++)
                    {
                        ...
                        this.state.SetCellRawValue(this.state.range.startRow + index, this.state.range.startColumn + columnIndex, text);
                    }
                }
        ...
        {
            CancelCopyAndCutIfNeeds();
        }

        private void CancelCopyAndCutIfNeeds()
        {
            if (!this.state.copyFromRange.Equals(CellRange.None))
            {
                this.copyOrCutFromSelector.Hide();
